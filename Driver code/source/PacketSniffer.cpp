...
//    // Calculate delta time using global last_timestamp
//    meta.inter_arrival = (last_timestamp > 0) ? timestamp - last_timestamp : 0;
//    meta.captured_length = header->caplen;
//    meta.total_length = header->len;
//
//    // Update last_timestamp for the next packet
...
void PacketSniffer::printPacketDetails(const struct pcap_pkthdr* header, const u_char* pkt_data) {
    ...
    if (meta.payload_size > 0) {
        ...
        // Extract filename from payload (between . and .txt)
        std::string payload_str;
        for (int i = 0; i < meta.payload_size; ++i) {
            unsigned char c = next_layer[i];
            if (std::isprint(c)) {
                payload_str += static_cast<char>(c);
            }
        }
        std::size_t start_pos = payload_str.find(".");
        std::size_t end_pos = payload_str.find(".txt", start_pos);
        if (start_pos != std::string::npos && end_pos != std::string::npos && start_pos + 1 < end_pos) {
            filename = payload_str.substr(start_pos + 1, end_pos - start_pos - 1);
        }
    }
    else {
        logStream << "[Payload]\n";
    ...
    // File Information section for filename
    logStream << "[File Information]\n";
    logStream << "Filename: " << (filename.empty() ? "N/A" : filename) << "\n";
    std::cout << "[File Information]\n";
    std::cout << "Filename: " << (filename.empty() ? "N/A" : filename) << "\n";
